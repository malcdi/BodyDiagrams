...
(function( $ ){
  ...
  var methods = {
    ...
    'loadImage': function () {
      ...
      if(settings.aspect){
        settings.min_width=data.target_dimensions.ratio*settings.min_height;
      }
      setupCSS.apply(this);
    ...
    'mouseWheel': function (delta) {
      ...
      if (delta > 0) {
        ...
        //no zoom out when maxxed.
        if(getWidth.apply(this)==settings.min_width && getHeight.apply(this)==settings.min_height)
          return;
    ...
    'ignore': function() {
      if (settings.mousewheel && typeof(this.mousewheel) == 'function') {
        this.parent().unbind('mousewheel.panZoom', mouseWheelHandler);
      }
    },
    'listen': function() {
      if (settings.mousewheel && typeof(this.mousewheel) == 'function') {
        this.parent().bind('mousewheel.panZoom', mouseWheelHandler);
      }
    }
...
/*
    ...
    if (settings.doubl_click) { 
      data.bound_elements = data.bound_elements.add(settings.double_click);
    }
    if (settings.mousewheel && typeof(this.mousewheel) == 'function') {
      data.bound_elements = data.bound_elements.add(settings.zoomIn);
    }
  ...
  function setupBindings() {
    ...
    // bind up controls
    if (settings.zoomIn) { 
      setEventHandler(settings.zoomOut, eventData, data, 'zoomIn'); 
    }
    if (settings.zoomOut) { 
      setEventHandler(settings.zoomOut, eventData, data, 'zoomOut');  
    }
    if (settings.panUp) { 
      setEventHandler(settings.panDown, eventData, data, 'panUp'); 
    }
    if (settings.panDown) { 
      setEventHandler(settings.panDown, eventData, data, 'panDown'); 
    }
    if (settings.panLeft) { 
      setEventHandler(settings.panLeft, eventData, data, 'panLeft'); 
    }

    if (settings.panRight) {
      setEventHandler(settings.panRight, eventData, data, 'panRight'); 
    }

    if (settings.fit) { 
      setEventHandler(settings.fit, eventData, data, 'fit'); 
    }

    if (settings.destroy) {
      setEventHandler(settings.destroy, eventData, data, 'destroy'); 
    }
  ...
  function setEventHandler(settingsOption,  eventData, data, name){
    if(name=='fit' || name=='destroy'){
      settingsOption.bind('click.panZoom', eventData, function(event) { 
        event.preventDefault(); event.data.target.panZoom(name); } ); 
      data.bound_elements = data.bound_elements.add(settingsOption);
    }
    else{
      settingsOption.bind('mousedown.panZoom', eventData, function(event) { 
        event.preventDefault(); event.data.target.panZoom('mouseDown', name); 
  ...
  function setupCSS() {
    ...
    var centering_left=(this.parent().width()-getWidth.apply(this))/2;
    this.css({
      'position': 'absolute',
      'top': 0,
      'left': centering_left
    });
    this.data('panZoom').position.x1=centering_left;
